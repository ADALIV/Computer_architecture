...
Fully-associative cache 이용
cache : memory = 1 : 4
*/

cacheline cache[0x10000]; // 캐시라인 64K개 = 2^16개
...
void cache_update(int tag) {
    ...
    if (cache_idx == -1) {
        while (1) {
            if (cache[oldest].sca_bit == 0) {
                // 오래된 캐시라인 발견
                cache_idx = oldest;
                oldest = (oldest == 0xffff) ? 0 : oldest+1;
                break;
            } else if (cache[oldest].sca_bit == 1) {
                // 오래된 캐시라인이나, 찬스를 하나 가진 경우
                cache[oldest].sca_bit = 0;
                oldest = (oldest == 0xffff) ? 0 : oldest+1;
            }
        }
    }
    cache[cache_idx].tag = tag;
    cache[cache_idx].valid = true;
    for (int i=0; i<16; i++) {
        cache[cache_idx].data[i] = ReadMEM((tag<<4) + i);
    }
}

void IF(MEMWB_latch *in, IFID_latch *out) {
    out->PC = in->PC + 16;
    out->inst = ReadMEM(out->PC/4);
...
void MEM(EXMEM_latch *in, MEMWB_latch *out) {
    ...
    if ((in->opcode == 0x0) && (in->funct != 0x8)) {
        ...
        // no operation
    } else if (in->MEM_read) { //lw
        d_hazard = true;
        out->R_return = in->R_write;
        out->R_write = in->R_write;
        int tag = (in->ALU_result) >> 6;
        int off = (in->ALU_result & 0x3f) >> 2;
        for (int i=0; i<0x10000; i++) {
            if (cache[i].tag == tag && cache[i].valid) {
                // hit
                // 메모리 주소의 상위 26bit
                out->R_write_value = cache[i].data[off];
                cache[i].sca_bit = 1;
                miss = false;
                st.no_hit++;
                break;
        ...
        if (miss) {
            switch (in->opcode) {
            case 0x24:
                out->R_write_value = ReadMEM(in->ALU_result/4) & 0xff;
                // out->R_write_value = memory[in->ALU_result/4] & 0xff;
                break;
            case 0x25:
                out->R_write_value = ReadMEM(in->ALU_result/4) & 0xffff;
                // out->R_write_value = memory[in->ALU_result/4] & 0xffff;
                break;
            default:
                out->R_write_value = ReadMEM(in->ALU_result/4);
        ...
        switch (in->opcode) {
            case 0x28:
                memory[in->ALU_result/4] = (ReadMEM(in->ALU_result/4) & 0xffffff00) | (in->R_write & 0xff);
                break;
            case 0x29:
                memory[in->ALU_result/4] = (ReadMEM(in->ALU_result/4) & 0xffff0000) | (in->R_write & 0xffff);
                break;
            default:
        ...
        for (int i=0; i<0x10000; i++) {
            if (cache[i].tag == tag && cache[i].valid) {
                ...
                switch (in->opcode) {
                case 0x28:
                    cache[i].data[off] = (ReadMEM(in->ALU_result/4) & 0xffffff00) | (in->R_write & 0xff);
                    break;
                case 0x29:
                    cache[i].data[off] = (ReadMEM(in->ALU_result/4) & 0xffff0000) | (in->R_write & 0xffff);
                    break;
                default:
                    cache[i].data[off] = in->R_write;
                    break;
                }
                cache[i].sca_bit = 1;
                miss = false;
...
int main(int argc, char *argv[]) {
    ...
    *RA = 0xffffffff; // ra(return address)
    ...
    printf("\nNumber of cycles : %d\
            ...
            st.no_cycles, st.no_inst, st.no_mem, st.no_reg, st.no_branch, st.no_branch_not, st.no_jump, st.no_hit, st.no_miss);
...
void control_hazard() {
    ...
    IFID[1].PC = addr;
    IFID[1].inst = ReadMEM(addr/4);
    ID(&IFID[1], &IDEX[0]);

    IDEX[1] = IDEX[0];
    IFID[1].PC = addr + 4;
    IFID[1].inst = ReadMEM(addr/4 + 1);
    EX(&IDEX[1], &EXMEM[0]);
    ID(&IFID[1], &IDEX[0]);
